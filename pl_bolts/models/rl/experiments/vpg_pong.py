...
from pl_bolts.models.rl.common import cli, wrappers
...
class Reinforce(pl.LightningModule):
    ...
    def __init__(self, env: str, gamma: float = 0.99, lr: float = 1e-4, batch_size: int = 32,
        ...
        self.env = [self.make_env() for _ in range(num_envs)]
        self.net = CNN(self.env[0].observation_space.shape, self.env[0].action_space.n)
        ...
        self.mean_rewards = 0
        self.reward_sum = 0.0
        self.baseline_buf = MeanBuffer(1000000)
    ...
    def train_batch(self) -> Tuple[List[torch.Tensor], List[torch.Tensor], List[torch.Tensor]]:
        ...
        for step_idx, exp in enumerate(self.exp_source):
            ...
            self.baseline_buf.add(exp.reward)
            baseline = self.baseline_buf.mean()
            scaled_reward = exp.reward - baseline
    ...
    def configure_optimizers(self) -> List[Optimizer]:
        ...
        optimizer = optim.Adam(self.net.parameters(), lr=self.lr, eps=1e-3)
        return [optimizer]
...
if __name__ == '__main__':
    ...
    trainer = pl.Trainer.from_argparse_args(args, gradient_clip_val=0.1)
    trainer.fit(model)

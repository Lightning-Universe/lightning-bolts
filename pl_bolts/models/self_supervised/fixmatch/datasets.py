...
def get_dataset(data_path, dataset, mode='fixmatch', batch_size=128, eval_step=1024,
                expand_labels=True):
    assert mode in ['fixmatch', 'comatch']
    base_dataset = MAP_DATASET[dataset](data_path, train=True,
                                        download=True)
    train_labeled_idxs, train_unlabeled_idxs = x_u_split(dataset,
                                                         base_dataset.targets,
                                                         eval_step=eval_step,
                                                         expand_labels=expand_labels,
                                                         batch_size=batch_size)
    train_labeled_dataset = MAP_SSL_DATASET[dataset](
        data_path, train_labeled_idxs, train=True,
        transform=TransformSSL(dataset, 'casual'))
    train_unlabeled_dataset = MAP_SSL_DATASET[dataset](
        data_path, train_unlabeled_idxs, train=True,
        transform=TransformSSL(dataset, mode))
    test_dataset = MAP_DATASET[dataset](data_path, train=False,
                                        transform=TransformSSL(dataset, mode).normalize)
    return train_labeled_dataset, train_unlabeled_dataset, test_dataset


class SSLDataModule(LightningDataModule):
    def __init__(self, data_path, dataset, mode='fixmatch', batch_size=128,
                 eval_step=1024, expand_labels=True, **kwargs):
        self.batch_size = batch_size
        self.train_labeled_dataset, self.train_unlabeled_dataset, self.test_dataset = get_dataset(data_path,
                                                                                                  dataset,
                                                                                                  mode, batch_size,
                                                                                                  eval_step,
                                                                                                  expand_labels)
    ...
    def train_dataloader(self):
        labeled_loader = DataLoader(self.train_labeled_dataset,
                                    batch_size=self.batch_size,
                                    pin_memory=False, num_workers=0, drop_last=True)
        unlabeled_loader = DataLoader(self.train_unlabeled_dataset,
                                      batch_size=self.batch_size,
                                      pin_memory=False, num_workers=0, drop_last=True)

...
class BasicBlockPreAct(nn.Module):
    def __init__(self, in_chan, out_chan, drop_rate=0, stride=1, pre_res_act=False):
        ...
        if in_chan != out_chan or stride != 1:
            self.downsample = nn.Conv2d(in_chan, out_chan, kernel_size=1, stride=stride, bias=False)
...
class WideResnetBackbone(nn.Module):
    ...
    def create_layer(self, input_channel, out_channel, bnum, stride=1, drop_rate=0, pre_res_act=False):
        layers = [
            BasicBlockPreAct(input_channel, out_channel, drop_rate=drop_rate, stride=stride, pre_res_act=pre_res_act),
        ]

...
class CoMatch(FixMatch):
    ...
    def training_step(self, batch, batch_idx):
        ...
        images = torch.cat([
            img_x_weak, img_u_weak, img_u_strong0, img_u_strong1
        ], dim=0)
        ...
        logits_u_weak, logits_u_strong0, logits_u_strong1 = torch.split(logits[batch_size:],
                                                                        unlabeled_batch_size)
        # Split features
        features_x = features[:batch_size]

        features_u_weak, features_u_strong0, features_u_strong1 = torch.split(features[batch_size:],
                                                                              unlabeled_batch_size)

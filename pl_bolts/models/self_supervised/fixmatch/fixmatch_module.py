...
class FixMatch(LightningModule):
    def __init__(self, ema_eval=True,
                 **kwargs):
    ...
    def setup(self, stage):
        if stage == 'fit':
            ...
            self.model = WideResnet(n_classes=n_classes,
                                    k=self.hparams.wresnet_k,
                                    n=self.hparams.wresnet_n)
            if self.ema_eval:
                self.ema_model = get_ema_model(self.model)
            self.total_steps = (
                    (len(train_loader['labeled'].dataset) // (
                            self.hparams.batch_size * max(1, self.hparams.gpus))) * float(
    ...
    def add_model_specific_args(parent_parser):  # pragma: no-cover
        ...
        parser.add_argument('--pseudo-thr', type=float, default=0.95,
                            help='pseudo label threshold')
        parser.add_argument('--coefficient-u', type=float, default=1.,
...
def cli_main():
    ...
    parent_parser.add_argument("--dataset", default="cifar10", type=str,
    ...
    parser.set_defaults(deterministic=True, max_epochs=200)
    args = parser.parse_args()
    dm = SSLDataModule(args.data_path, args.dataset, batch_size=args.batch_size,
                       eval_step=args.eval_step, expand_labels=args.expand_labels)

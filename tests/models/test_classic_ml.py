...
def test_linear_regression_model(tmpdir):
    ...
    trainer = pl.Trainer(max_epochs=300, default_root_dir=tmpdir, progress_bar_refresh_rate=0, logger=False)
    trainer.fit(
        model,
        loader,
        loader,
...
def test_logistic_regression_model(tmpdir, datadir):
    ...
    trainer = pl.Trainer(max_epochs=3, default_root_dir=tmpdir, progress_bar_refresh_rate=0, logger=False,)
    trainer.fit(model)
    trainer.test(model)

...
def to_tensor(integers: np.ndarray) -> torch.Tensor:
...
class TestArrayDataset:
    ...
    def array_dataset(self):
        features_1 = DataModel(
            data=np.array([[1, 0, -1, 2], [1, 0, -2, -1], [2, 5, 0, 3], [-7, 1, 2, 2]]), transform=add_one_to_tensor
        )
        target_1 = DataModel(data=np.array([1, 0, 0, 1]), transform=to_tensor)
